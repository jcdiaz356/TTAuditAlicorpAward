...
//        }
     ...
     * @return
     */
    public static ArrayList<Store> getListStores(int user_id, int company_id, int level){
        int success ;
        ...
        try {
            ...
            JSONObject json = jsonParser.makeHttpRequest(GlobalConstant.dominio + "/admin_api/api_JsonRoadsDetailMarket.php" ,"POST", params);
            // check your log for json response
            Log.d("Login attempt", json.toString());
            // json success, tag que retorna el json
            if (json == null) {
                ...
                if (success > 0) {
                    ...
                    if(ObjJson.length() > 0) {
                        ...
                        for (int i = 0; i < ObjJson.length(); i++) {
                            ...
                            if(obj.isNull("store_id_parent")) store.setStore_id_parent(0);  else store.setStore_id_parent(obj.getInt("store_id_parent"));;
                            store.setLevel(obj.getInt("nivel"));
