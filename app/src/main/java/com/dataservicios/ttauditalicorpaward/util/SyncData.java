...
public class SyncData extends AsyncTask<Void, String, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        publishProgress(context.getString(R.string.text_download_store_parent));
        //for(Route r: routes){
            stores = AuditUtil.getListStores(user_id,company_id,1);
            for (Store s: stores) {
                storeRepo.create(s);
        ...
        //for(Route r: routes){
        stores = AuditUtil.getListStores(user_id,company_id,0);
        for (Store s: stores) {
            storeRepo.create(s);
            Log.d(LOG_TAG,s.toString());
        }
