package com.dataservicios.ttauditalicorpaward.repo;

import android.content.Context;

import com.dataservicios.ttauditalicorpaward.db.DatabaseHelper;
...
public class StoreRepo implements Crud {
    ...
    public List<Store> findByRouteIdAndLevel(int route_id, int level) {

        List<Store> wishList = null;
        try {
            wishList = helper.getStoreDao().queryBuilder().where().eq("route_id",route_id).and().eq("level",level).query();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return wishList;
    ...
    public List<Store> findByRouteIdAndStoreIdParent(int route_id, int store_id_parent) {

        List<Store> wishList = null;
        try {
            wishList = helper.getStoreDao().queryBuilder().where().eq("route_id",route_id).and().eq("store_id_parent",store_id_parent).query();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return wishList;
